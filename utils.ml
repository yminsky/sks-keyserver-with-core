...
let rec extract_words_rec s ~start ~len partial =
  let one () = Set.add partial (String.lowercase (String.sub s start len)) in
...
let extract_word_set s =
  extract_words_rec s ~start:0 ~len:0 String.Set.empty
...
let initdbconf src dst =
  ...
  let conf_exists conf = 
    Sys.file_exists_exn (Filename.concat src conf)
  in
  let found_conf = List.filter lstconf
                     ~f:(fun x -> conf_exists x)  
  in
