...
let hexa_digit x =
  if x >= 10 then Char.of_int_exn (Char.to_int 'A' + x - 10)
  else Char.of_int_exn (Char.to_int '0' + x)

let hexa_val conf =
  match conf with
    '0'..'9' -> Char.to_int conf - Char.to_int '0'
  | 'a'..'f' -> Char.to_int conf - Char.to_int 'a' + 10
  | 'A'..'F' -> Char.to_int conf - Char.to_int 'A' + 10
  | _ -> 0
      ...
      let i1 =
        ...
        | c ->
          if special c then
            begin
              s1.[i1] <- '%';
              s1.[i1 + 1] <- hexa_digit (Char.to_int c / 16);
              s1.[i1 + 2] <- hexa_digit (Char.to_int c mod 16);
              i1 + 3
            end
...
let stripchars = Char.Set.of_list [ ' '; '\t'; '\n'; '\r' ]

let strip s =
  let start = ref 0 in
  while (!start < String.length s
         && Set.mem stripchars s.[!start]) do
    incr start
  done;
  let stop = ref (String.length s - 1) in
  while (!stop >= 0 && Set.mem stripchars s.[!stop]) do
    decr stop
  done;
  if !stop >= !start then
...
let parse_post headers cin =
  try
    let lengthstr = Map.find_exn headers "content-length" in
    let len = Int.of_string lengthstr in
...
let parse_request cin =
  ...
  let headers = parse_headers String.Map.empty cin in
  match List.hd_exn pieces with
  | "GET"  -> GET  (List.nth_exn pieces 1, headers)
  | "POST" -> POST (List.nth_exn pieces 1, headers, parse_post headers cin)
...
let request_to_string request =
  let (kind,req,headers) =
    match request with
    | GET  (req,header_map)   -> ("GET",  req, headers_to_string header_map)
    | POST (req,header_map,_) -> ("POST", req, headers_to_string header_map)
  in
