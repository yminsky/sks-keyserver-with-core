...
let rec join_by_keypacket map keylist = match keylist with
  ...
  | key::tl ->
    ...
    let map =
      try
        let keylist_ref = Map.find_exn map keypacket in
        keylist_ref := key::!keylist_ref;
        map
      with
      | Not_found ->
        Map.add map ~key:keypacket ~data:(ref [key])
    in
...
let join_by_keypacket keys =
  Map.fold ~f:(fun ~key ~data list -> !data::list) ~init:[]
    (join_by_keypacket Map.Poly.empty keys)
...
let presentation_filter key =
  ...
  then None
    ...
    let subkeys = List.filter_map ~f:sig_filter_sigpair pkey.subkeys in
    let uids = List.filter_map ~f:sig_filter_sigpair pkey.uids in
